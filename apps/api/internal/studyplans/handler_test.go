...
func setupRouter(t *testing.T, svc studyPlanServicer) (*gin.Engine, *auth.JWTManager, uuid.UUID) {
	t.Helper()
...
func TestCreateStudyPlanRequiresAuth(t *testing.T) {
	r, _, _ := setupRouter(t, newFakeStudyPlanService())
	body, _ := json.Marshal(map[string]any{"title": "Plan 1"})
